...
## 📋 Assumptions Made
- **Local Sandbox Storage**: It is assumed that the Supabase storage bucket `incident-attachments` is public and permits direct inserts from anonymous client uploads. For production use, a signed upload policy is recommended.
- **Pre-seeded Authentication**: Hashed default credentials for Bob (Staff) and Alice (Manager) are stored directly in the `users` table upon running the schema script, matching pre-calculated SHA256 hashes generated by standard cryptographic hashing.
- **Soft-deletes and Persistence**: Incidents are kept in the database indefinitely to maintain complete dashboard metrics and trends. Closed or resolved incidents are simply status changes rather than database deletions.
- **Groq API Inference Optionality**: The AI incident auto-classification features are treated as optional. If the API key is missing or the Groq service fails, the form still allows manual categorization and is fully submit-ready.
...
- **shadcn/ui custom blocks**: Tailored styling components (Card, Button, Input, Table, Dialog, Badge, Skeleton) derived from Radix primitives and styled with vanilla Tailwind CSS.
